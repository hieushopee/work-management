import { Building2, UsersRound, Users, ChevronLeft, ChevronRight, UserCog } from 'lucide-react';
...
const items = [
  { id: 'profiles', label: 'Employee Profiles', icon: Users, path: '/employees/profiles' },
  { id: 'departments', label: 'Department Management', icon: Building2, path: '/employees/departments' },
  { id: 'teams', label: 'Team Management', icon: UsersRound, path: '/employees/teams' },
  { id: 'accounts', label: 'Account Management', icon: UserCog, path: '/employees/accounts', adminOnly: true },
];

const UserModuleSidebar = () => {
  const { pathname } = useLocation();
  const { user } = useUserStore();
  const [collapsed, setCollapsed] = useState(() => {
    return localStorage.getItem('userModuleSidebarCollapsed') === 'true';
  });

  useEffect(() => {
    localStorage.setItem('userModuleSidebarCollapsed', collapsed ? 'true' : 'false');
  }, [collapsed]);
      ...
      <nav className="flex-1 p-4">
        <ul className="space-y-1">
          {visibleItems.map((item) => {
            ...
            const isEmployeesRoot = pathname === '/employees' || pathname === '/employees/';
            const isActive = pathname.startsWith(item.path) || (isEmployeesRoot && item.id === 'profiles');
...
export default UserModuleSidebar;
