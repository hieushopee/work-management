import { CheckSquare, FileText, Clock, ChevronLeft, ChevronRight } from "lucide-react";
import { Link, useLocation } from "react-router-dom";
import { useState, useEffect } from 'react';

const items = [
  { id: "todo", label: "To-do List", icon: CheckSquare, path: "/calendar/manage/todo" },
  { id: "report", label: "Report", icon: FileText, path: "/calendar/manage/report" },
  { id: "work-hours", label: "Work Hours", icon: Clock, path: "/calendar/manage/work-hours" },
];

const CalendarModuleSidebar = () => {
  const { pathname } = useLocation();
  const [collapsed, setCollapsed] = useState(() => {
    return localStorage.getItem('calendarModuleSidebarCollapsed') === 'true';
  });

  useEffect(() => {
    localStorage.setItem('calendarModuleSidebarCollapsed', collapsed ? 'true' : 'false');
  }, [collapsed]);
...
export default CalendarModuleSidebar;
